...
export function getKeysGPG() {
  if ((os.platform() === 'linux' || os.platform() === 'darwin') && !process.env.PATH.includes('/usr/local/bin')) {
    process.env.PATH += ":/usr/local/bin";
  }

  const output = child_process.execSync('gpg --list-secret-keys --fingerprint').toString();
  ...
  for (let line of output.split('\n')) {
    ...
    if (line.startsWith('sec') || line.startsWith('ssb')) {
      ...
      let parsed = /^(sec|ssb) +(?:\w+)?([0-9]*)(?:[a-zA-Z]?)\/([a-zA-Z0-9]*) ((2[0-9]{3})-(1[0-2]|0[1-9])-(0[1-9]|[12]\d|3[01]))(?: \[expires\: )?((2[0-9]{3})-(1[0-2]|0[1-9])-(0[1-9]|[12]\d|3[01]))?/.exec(line).slice(1);
