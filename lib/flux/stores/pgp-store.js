...
import FileDownloadStoreWatcher from '../file-download-store-watcher';
...
class PGPStore extends NylasStore {
  ...
  _getKey() {
    const keyLocation = path.join(process.env.HOME, 'pgpkey');
  ...
  retrievePGPAttachment(message, notify) {
    ...
    if (dataPart) {
      ...
      return fs.accessAsync(filePath, fs.F_OK | fs.R_OK).then(() =>
        fs.readFileAsync(filePath, 'utf8').then((text) => {
        ...
        return FileDownloadStoreWatcher.promiseForPendingFile(dataPart.id);
      });
    }
  ...
  getAttachmentAndKey(message, notify) {
    const keys = {};
    const gpgKeys = KeyStore.getKeysGPG();
    ...
    const promises = [
      ...
      smalltalk.dropdown('PGP Key', dropdownQuestion, keys)
        .then(null, () => Promise.reject(new FlowError('Cancelled key selection', true)))
    ];
    return Promise.all(promises).spread((text, pgpkey) => {
      if (!text) {
        return Promise.reject(new FlowError('No text in attachment', true));
      }
      if (!pgpkey) {
  ...
  _decryptAndResetCache(message) {
    const key = MessageBodyProcessor._key(message);
    ...
    return this.mainDecrypt(message).then(() => {
      if (this._state.has(message.id) && !this._state.get(message.id).lastError) {
        ...
        const processed = MessageBodyProcessor.process(message);
