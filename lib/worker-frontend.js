  ...
  _forkProcess() {
    ...
    this._child.on('message', (message) => {
      if (message.method === proto.ERROR_OCCURRED) {
        ...
        this.log.error('Error from worker: %s %s', error, error.childStackTrace);
      } else if (message.method === proto.VERBOSE_OUT) {
        // VERBOSE_OUT
        this.log.info('[Verbose] %s', message.message);
        ...
        this.log.info('Unknown Message Received From Worker: %O', message);
      }
    });
  }
